...
public class LysimeterFirst {
	
	// TRANSPIRATION PARAMETERS 
	@Description("The ET every tTimestep.")
	@In
	@Unit("mm/s")
	public HashMap<Integer, double[]> inTranspiration;
	
	@Description("The ET divided for the tTimestep.This is the transpiration without the stress factor")
	@In
	@Unit("m/s")
	double transpiration;
	
	@Description("Water transpired divided for the rootsDepth")
	@In
	@Unit ("1/s")
	public double transpiredWater;	
	
	@Description("Potential transpiration from each layer ")
	@Out
	@Unit("mm/s")
	public double[] transpiredWaters;	
	
	@Description("Potential transpiration from each layer ")
	@Out
	@Unit("m")
	public double[] transpiredWaterForLayers;
	
	@Description("The Stress Factor is a coefficient thet describes the condiction of stress of canopy")
	@Out
	@Unit("-")
	public double[] stressFactor;
	
	@Description("The sumTranspiredStressWater is the sum of the transpiredStressWaterForLayers")
	@Out
	@Unit("m")
	public double sumTranspiredStressWater;
		
	@Description("Effective transpiration from each layer")
	@Out
	@Unit("m")
	public double[] transpiredStressWaterForLayers;
	
	@Description("Potential transpiration from each layer ")
	@Out
	@Unit("mm/s")
	public double[] transpiredStressWaters;	
	
	@Description("The outET is the stressed traspiration")
	@Out
	@Unit("mm/s")
	public double outET;
	...
	@Description("Dimension of layers")
	@In
	@Unit("m")
	double[] layers;
		
	@Description("Number of layer of the domain discetrization")
	@Unit ("-") 
	int numberOfLayers;
	
	@Description("Minimum value of water, volumes-thetaR, so this is only theta without residual water.")
	@Unit ("m")
	double minValue;
	...
	public void solve() throws SchemaException {
		...
		if(step==0){
			...
			layers         = new double[NUM_CONTROL_VOLUMES];
			stressFactor   = new double[NUM_CONTROL_VOLUMES];
		...
		transpiration = 0.0; 
		transpiration = (inTranspiration.get(1)[0]/1000)/tTimestep;
		...
		while(sumTimeDelta < tTimestep) {
			...
			for(int picard=0; picard<picardIteration; picard++) {
				...
				numberOfLayers     = z.length; //E'LA STESSA COSA DI NUM_CONTROL_VOLUMES, QUINDI QUESTA VARIABILE SI PUò ANCHE ELIMINARE
				transpiredWaterForLayers = new double[numberOfLayers];
				transpiredWaters   = new double[numberOfLayers];
				
				transpiredWater    = transpiration/rootsDepth; 
			
				for(int i = 0; i < NUM_CONTROL_VOLUMES; i++) {layers[i] = dx[i];}

				for(int i = 0; i <NUM_CONTROL_VOLUMES; i++) {
					if(totalDepth < rootsDepth) {transpiredWaterForLayers[i] = transpiredWater*dx[i]*timeDelta;} 
					else {transpiredWaterForLayers[i] = 0;}
				    totalDepth = totalDepth-layers[i];
				    transpiredWaters[i] = transpiredWaterForLayers[i]*1000/timeDelta;
				}
				
			
				transpiredStressWaters = new double[numberOfLayers];
				transpiredStressWaterForLayers = new double[numberOfLayers];
				
				sumTranspiredStressWater  = 0;			
				for(int i = 0; i < NUM_CONTROL_VOLUMES-2; i++) {
					stressFactor[i] = (volumes[i]/dx[i] - thetaWp)/ (thetaFc - thetaWp); 
					stressFactor[i] = ((stressFactor[i]>0)?stressFactor[i]:0);				
					stressFactor[i] = ((stressFactor[i]<1)?stressFactor[i]:1);
					minValue = volumes[i]-thetaR[i] * dx[i];
					transpiredStressWaterForLayers[i] = transpiredWaterForLayers[i] * stressFactor[i]; 
					transpiredStressWaterForLayers[i] = ((transpiredStressWaterForLayers[i]<minValue)?transpiredStressWaterForLayers[i]:minValue);//+(thetaR[i]*0.2*dx[i]));
					transpiredStressWaters[i] = transpiredStressWaterForLayers[i]*1000/timeDelta;
					sumTranspiredStressWater  = sumTranspiredStressWater + transpiredStressWaterForLayers[i];
				}
				
				outET = sumTranspiredStressWater*1000/timeDelta;
				...
				for(int i = 0; i < NUM_CONTROL_VOLUMES; i++) {
					if( i == 0 ) {
						...
						if(bottomBCType.equalsIgnoreCase("Bottom Free Drainage") || bottomBCType.equalsIgnoreCase("BottomFreeDrainage")){
							...
							kM = -999;}
						else {kM = interfaceHydraulicConductivity.compute(kappas[i],k_b,dx[i],dx[i]);
						}
						...
						rhss[i] = volumes[i]  + bottomBoundaryCondition.rightHandSide(bottomBC, kP, kM, spaceDelta[i+1], spaceDelta[i], timeDelta, delta) - transpiredStressWaterForLayers[i];} 
						...
						rhss[i] = volumes[i] + topBoundaryCondition.rightHandSide(topBC, kP, kM,  spaceDelta[i], spaceDelta[i-1], timeDelta, delta) - transpiredStressWaterForLayers[i];} 
						...
						rhss[i] = volumes[i] + timeDelta*(kP - kM) - transpiredStressWaterForLayers[i];}	
					}
				
				System.out.println("ET stress mm/s			"+outET);
				System.out.println("");
				...
				// COMPUTE velocities AT CELL INTERFACES at time level n+1 with hydraulic conductivity at time level n 
				...
				errorVolume = volumeNew - volume - timeDelta*(topBC + velocities[0]- outET/1000);
			}
		...
		outputToBuffer.add(stressFactor);
		outputToBuffer.add(transpiredWaters);
		outputToBuffer.add(transpiredStressWaters);
